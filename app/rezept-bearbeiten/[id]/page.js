	...
	return (
		<>
			<div className="container mx-auto mb-10 max-w-4xl rounded-(--border-radius-md) bg-(--background-light) px-6 py-10 shadow-(--box-shadow-light)">
				<h1 className="text-2xl font-bold mb-6">Dein Rezept</h1>
				<form encType="multipart/form-data" onSubmit={handleSubmit(submitHandler)} noValidate={true}>
					...
					<button
						className="btn cursor-pointer rounded bg-[#4CAF50] px-3.75 py-3.75 text-white"
						type="submit"
					>
						Rezept aktualisieren
					</button>
					<Link className="btn btn--primary float-right" href="/meine-rezepte">Abbrechen</Link>
				</form>
			</div>
		</>
	)
